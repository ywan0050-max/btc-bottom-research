...
本项目软件源代码采用 `AGPL-3.0-only`，完整条款见 [LICENSE](LICENSE)。软件许可证与第三方数据条款是两件独立的事情：AGPL 不会覆盖或改变数据提供方的许可。仓库默认不包含运行时下载的 DuckDB 或 Parquet 数据；使用者直接从各公开来源获取数据，并应自行遵守来源的最新条款。
