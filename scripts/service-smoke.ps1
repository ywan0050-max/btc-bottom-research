...
try {
    ...
    do {
        if ($Server.HasExited) {
            ...
            throw "Service exited before becoming healthy with code $($Server.ExitCode). $ErrorText"
        }
    ...
    $ErrorText = $Server.StandardError.ReadToEnd()
    if ($OutputText -notmatch "指定端口 $OccupiedPort 已被占用") {
        throw "The launcher did not clearly report the occupied requested port. Output: $OutputText Error: $ErrorText"
    }
    ...
    $StrictProcessInfo.FileName = "powershell.exe"
